...
작성일: 2026-09-15 (Gmail 수신본 확인 후 레이아웃 수정 반영)
...
### 2-4. Gmail 수신본에서 PICK 레이아웃이 깨졌다 — 구조 교체

실제 Gmail 수신본을 확인하니 PICK 제목과 흰 카드가 매장 사진 위로 겹치지 않고
**사진 아래로 떨어져 나왔다.** 원인은 Gmail 이 `position`과 음수 `margin`을 제거하기
때문이다. 배포된 템플릿에 그 둘이 각각 쓰이고 있었다.

사진을 `<td>`의 배경 이미지로 깔고 그 안에 제목과 카드를 평범한 표로 쌓는 구조로
바꿨다. 겹침을 배치가 아니라 배경으로 해결하므로 Gmail 에서도 시안대로 보인다.
**치환 토큰은 그대로라 백엔드 코드 변경은 없다.**

### 2-5. 이미지 슬롯을 전부 정사각으로 바꿨다

서버가 주는 SKU 이미지는 **1500 x 1500 정사각**인데 시안의 슬롯은 가로형이었다
(추천 1번이 299 x 186). Gmail 은 `object-fit`도 지원하지 않아 정사각 이미지가
가로로 **61% 늘어난 채** 나갔다.

슬롯을 원본 비율에 맞춰 정사각으로 바꿨다.

| 슬롯 | 이전 | 현재 |
| --- | --- | --- |
| PICK 1~4 | 148.6 x 161 | **149 x 149** |
| 추천 1 | 299 x 186 | **186 x 186** |
| 추천 2 | 208 x 196 | **196 x 196** |

**SKU 이미지를 정사각이 아닌 비율로 바꿀 계획이 있으면 미리 알려달라.** 슬롯 크기를
다시 맞춰야 한다. `object-fit:cover`는 지원하는 클라이언트를 위해 남겨 뒀지만
Gmail 에서는 동작하지 않으므로 그것만 믿을 수 없다.

### 2-6. 확인 부탁 — 상품명에 HTML 엔티티가 섞여 있는 것 같다

Gmail 수신본에서 상품명이 `모노그램 미디스커트 ECONYL&reg;`로 보였다. `®`가 아니라
엔티티 문자열이 그대로 노출된 것이다. 상품명 데이터에 `&reg;`가 **문자 그대로**
들어 있고, 서버가 출력할 때 `&`를 올바르게 이스케이프해서 생긴 결과로 보인다.

무작위 태그로 표본 스무 개 남짓 뽑아봤지만 해당 상품이 걸리지 않아 재현은 못 했다.
상품명 필드에 엔티티 문자열이 저장된 건이 있는지 확인해 달라 — 있다면 데이터 쪽에서
실제 문자(`®`)로 바꾸는 게 맞다.
