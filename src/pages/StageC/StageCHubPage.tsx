...
export default function StageCHubPage({ screenId }: StageCHubPageProps) {
  ...
  const headline = isProductIntro
    ? [`MCM의 대표 제품 ${product.name}이네요`, '어떤 점이 궁금하신가요?']
    : screen.heading;

  return (
    <div className="relative min-h-dvh w-full overflow-hidden bg-black">
      <img src={backgroundImage} alt="" className="absolute inset-0 h-full w-full object-cover" />
      <div className="relative z-10 mx-auto flex min-h-dvh w-full max-w-100.5 flex-col items-center px-[7.71%] pt-12 pb-16.25">
        <span className="rounded-full bg-[#d9d9d9]/15 px-4 py-1 text-[12.5px] text-[#d1d1d1]">{product.name}</span>
        <img src={product.imageUrl} alt={product.name} className="mt-4 h-40 w-full rounded-[15px] object-cover" />
        <div className="mt-6 flex w-full flex-col items-center gap-4">
          {screen.intro && <p className="text-[14px] text-[#d1d1d1]">{screen.intro}</p>}
          <ScreenHeadline headline={headline} variant="md" />
          <div className="mt-2 flex w-full flex-col gap-2.75">
            {screen.choices.map((choice) => (
              <SecondaryButton key={choice.id} label={choice.label} onClick={() => selectChoice(choice)} />
            ))}
          </div>
        </div>
        <div className="mt-auto flex w-full flex-col gap-3.25">
          <PrimaryButton label={screen.purchaseActionLabel} onClick={requestPurchase} />
