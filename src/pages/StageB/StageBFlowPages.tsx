...
export function StageBNfcPromptPage() {
  ...
  return (
    <div className="relative">
      <B1NfcPrompt onCallStaff={() => dispatch({ type: SESSION_ACTIONS.setActiveOverlay, overlay: 'E' })} />
      {/* 임시 트리거: 실제 NFC 스캔 로직 전까지 버튼으로 태그 인식을 흉내낸다. */}
      <button
        type="button"
        onClick={() => navigate(stageBRecognizingPath(DEFAULT_PRODUCT_SKU))}
        className="fixed bottom-6 left-5 z-40 rounded-full bg-white/10 px-4 py-2 text-[12px] text-white/70"
      >
        [임시] NFC 인식
      </button>
    </div>
